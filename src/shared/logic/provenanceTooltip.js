...
export function getTooltipAnchorProps(tooltip, content, options = {}) {
    ...
    if (typeof tooltip === 'object' && tooltip.show && tooltip.hide) {
        return {
            ...sharedProps,
            // Bubbling keeps child controls clickable while the row owns the
            // tooltip.
...
export function findTemporalRecordAtPointer(records, maxIndex, clientX, bounds) {
    ...
    // The native control sits before the plot, so hovering it resolves to the
    // first visible interval rather than losing the tooltip.
